...
package object barneshut {
  ...
  case class Fork(
    ...
    val massX: Float = if (mass == 0) centerX else quads.map(quad => quad.mass * quad.massX).sum / mass
    val massY: Float = if (mass == 0) centerY else quads.map(quad => quad.mass * quad.massY).sum / mass
